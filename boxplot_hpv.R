...
plot1 <-ggplot(gathered, aes(x=gene.name, y=expression.level, fill = hpv.status)) + 
...
for (i in c(1, 3:6, 8:14)) {
  res.aov <- aov(expression.level ~ hpv.status, data = gathered %>% filter(gene.name == paste('SLC2A',i,sep = "")))
...
ggsave('boxplot_hpv_FINALd.png',plot = g2, width = 345, height = 189, units = "mm", dpi = 300)